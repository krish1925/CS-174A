...
export class Transforms_Sandbox extends Transforms_Sandbox_Base {
    ...
    //    this.shapes.box.draw(context, program_state, model_transform, this.materials.plastic.override(blue));
        ...
        for (let i = 0; i < 8; i++) {
            ...
            if (!this.hover)
                model_transform = model_transform.times(Mat4.rotation(t, -1, 0, 0))
    ...
    //    this.shapes.box.draw(context, program_state, model_transform, this.materials.plastic.override(blue));
      ...
      //  this.shapes.ball.draw(context, program_state, model_transform, this.materials.metal.override(blue));
        ...
        //for (let i = 0; i < 8; i++) {
            // Apply transformations for each box
            model_transform = model_transform
                .times(Mat4.scale(2, 2, 2)) // Scale the box to dimensions 2x2x2
                .times(Mat4.translation(0, 2, 0)); // Translate each box by the height

            // Draw the box with the current model transform
            //this.shapes.box.draw(context, program_state, model_transform, this.materials.plastic.override(blue));
        //}
